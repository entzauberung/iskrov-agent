...
class RunController:
    ...
    async def _bridge_published_snapshot_id(self, scope: ExecutionScope) -> str:
        """Use one durable client current snapshot; never workspace-wide READY order."""
        return await self._bridge_client_current_snapshot_id(scope)
    ...
    async def _bridge_client_current_snapshot_id(self, scope: ExecutionScope) -> str:
        """Resolve one published client pointer without latest/first READY fallback."""
        clients = await self._store.list_bridge_clients(
            principal_id=scope.principal_id, workspace_id=scope.workspace_id
        )
        for client in sorted(clients, key=lambda item: item.client_id):
            if client.status is not BridgeClientStatus.ACTIVE:
                continue
            if not client.current_snapshot_id:
                continue
            try:
                snapshot = await self._store.get_snapshot(
                    client.current_snapshot_id, owner_id=scope.principal_id
                )
            except MissingEntityError:
                continue
            if (
                snapshot.status is SnapshotStatus.READY
                and snapshot.workspace_id == scope.workspace_id
            ):
