...
class LocalPatchStore:
    ...
    async def create_change_set(self, change_set: ChangeSet) -> ChangeSet:
        ...
        if existing:
            current = existing[0]
            if (
                current.run_id != change_set.run_id
                or current.workspace_id != change_set.workspace_id
                or current.base_snapshot_id != change_set.base_snapshot_id
                or current.patch != change_set.patch
            ):
                raise ValueError("tool call already has a different ChangeSet")
            return current
