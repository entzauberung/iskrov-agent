...
_STATE_VERSION = 1
_EXECUTION_RECORD_VERSION = 1
_LOCAL_WRITE_STATUSES = frozenset({"PENDING", "CONFIRMED", "UNKNOWN"})
_UNCERTAIN_WRITE_MESSAGE = (
    "a previous local write is uncertain and must not be rerun (UNKNOWN/BLOCKED)"
)
...
def _state_entries(
    ...
    for key, item in value.items():
        ...
        entry = dict(item)
        if field_name == "claimed_calls" and entry.get("execution") is not None:
            entry["execution"] = _validated_execution(entry["execution"])
        entries[key] = entry
    return entries
...
def _optional_execution_id(value: Any, field_name: str) -> str | None:
    if value is None:
        return None
    if not isinstance(value, str) or not value or _is_absolute_path(value):
        raise BridgeStateError(f"stored local execution {field_name} is invalid")
    return value


def _change_set_id_from_payload(payload: Mapping[str, Any]) -> str | None:
    candidates: list[Any] = [payload.get("change_set_id")]
    nested = payload.get("result")
    if isinstance(nested, Mapping):
        candidates.append(nested.get("change_set_id"))
    for item in candidates:
        if isinstance(item, str) and item and not _is_absolute_path(item):
            return item
    return None


def _execution_identity(
    claim: Mapping[str, Any], *, client_id: str | None, call_id: str
) -> dict[str, str]:
    identity = {"call_id": call_id}
    if isinstance(client_id, str) and client_id:
        identity["client_id"] = client_id
    for key in ("claim_id", "snapshot_id"):
        value = claim.get(key)
        if isinstance(value, str) and value and not _is_absolute_path(value):
            identity[key] = value
    return identity
...
def _bind_execution_identity(
    ...
    for key in ("call_id", "client_id", "claim_id", "snapshot_id"):
        stored = record.get(key)
        if stored is None:
            continue
        if identity.get(key) != stored:
            raise BridgeStateError(
                "stored local execution does not match the original claim"
            )


def _pending_execution(identity: Mapping[str, str]) -> dict[str, Any]:
    return {
        "version": _EXECUTION_RECORD_VERSION,
        "status": "PENDING",
        **dict(identity),
    }


def _confirmed_execution(
    identity: Mapping[str, str], payload: Mapping[str, Any]
) -> dict[str, Any]:
    record: dict[str, Any] = {
        "version": _EXECUTION_RECORD_VERSION,
        "status": "CONFIRMED",
    ...
    change_set_id = _change_set_id_from_payload(payload)
...
def _validated_execution(value: Any) -> dict[str, Any]:
    ...
    version = value.get("version", _EXECUTION_RECORD_VERSION)
    if version != _EXECUTION_RECORD_VERSION:
        raise BridgeStateError("unsupported local execution record version")
    status = value.get("status")
    payload = value.get("payload")
    fingerprint = value.get("fingerprint")
    if status is None:
        if isinstance(payload, dict) and isinstance(fingerprint, str):
            status = "CONFIRMED"
        else:
            raise BridgeStateError("stored local execution is incomplete")
    if status not in _LOCAL_WRITE_STATUSES:
        raise BridgeStateError("stored local execution status is invalid")
    record: dict[str, Any] = {
        "version": _EXECUTION_RECORD_VERSION,
        "status": status,
    }
    for key in ("call_id", "client_id", "claim_id", "snapshot_id", "change_set_id"):
        item = _optional_execution_id(value.get(key), key)
        if item is not None:
            record[key] = item
    if status == "CONFIRMED":
        ...
        change_set_id = record.get("change_set_id") or _change_set_id_from_payload(payload)
        if change_set_id is not None:
            record["change_set_id"] = change_set_id
    _assert_state_safe(record)
    return record
...
class Bridge:
    ...
    async def claim_tool_call(
        ...
        assigned_client_id = response.get("client_id")
        if assigned_client_id is not None and assigned_client_id != self._require_client_id():
            raise BridgeProtocolError("claim was assigned to a different Bridge client")
    ...
    async def execute_claim(
        ...
        claim_client_id = claim.get("client_id")
        if claim_client_id is not None and claim_client_id != self._require_client_id():
            raise BridgeProtocolError("claim belongs to a different Bridge client")
        identity = _execution_identity(
            claim, client_id=self.state.client_id, call_id=call_id
        )
        ...
        if stored_execution is not None:
            record = _validated_execution(stored_execution)
            _bind_execution_identity(record, identity)
            if record.get("status") in {"PENDING", "UNKNOWN"}:
                if record.get("status") == "PENDING":
                    ...
                    self._save_state()
                raise BridgeStateError(_UNCERTAIN_WRITE_MESSAGE)
            ...
            if str(claim.get("effect", "")).upper() == "WRITE":
                entry["execution"] = _pending_execution(identity)
                self._save_state()
            try:
                ...
                if str(claim.get("effect", "")).upper() == "WRITE":
                    raise BridgeStateError(_UNCERTAIN_WRITE_MESSAGE) from error
                raise
            ...
            entry["execution"] = _confirmed_execution(identity, payload)
            self._save_state()
