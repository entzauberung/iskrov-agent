...
CREATE TABLE IF NOT EXISTS bridge_clients (
    ...
    last_seen_at           TEXT,
    disabled_at            TEXT,
    current_snapshot_id    TEXT REFERENCES snapshots (snapshot_id) ON DELETE RESTRICT,
    UNIQUE (client_id, principal_id, workspace_id),
    CHECK (
        current_snapshot_id IS NULL
        OR (current_snapshot_id = TRIM(current_snapshot_id) AND current_snapshot_id <> '')
    ),
    CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_bridge_clients_current_snapshot
    ON bridge_clients (current_snapshot_id)
