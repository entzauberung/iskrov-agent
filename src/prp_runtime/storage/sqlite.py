...
class SqliteStore:
    ...
    async def get_active_bridge_claim(
        ...
        *,
        ...
        """Return the ACTIVE lease for one owner-scoped call, if any."""
        row = await self._fetch_one(
            """
            SELECT bc.*
              FROM bridge_claims AS bc
              JOIN workspaces AS w ON w.workspace_id = bc.workspace_id
             WHERE bc.call_id = ? AND bc.workspace_id = ? AND bc.owner_id = ?
               AND w.owner_id = ? AND bc.status = 'ACTIVE'
             ORDER BY bc.claimed_at DESC, bc.claim_id DESC
            """,
            (call_id, workspace_id, principal_id, principal_id),
        )
        if row is None:
            return None
    ...
    def bridge_client_liveness(
        ...
        *,
        ...
        row = self._durable_bridge_liveness_row(client_id)
        if row is None:
            self._bridge_heartbeats.pop(client_id, None)
            return BridgeClientLiveness.OFFLINE
        status = str(row["status"])
        last_seen = _parse_datetime(row["last_seen_at"])
        stored_principal = str(row["principal_id"])
        stored_fingerprint = str(row["capability_fingerprint"])
        if last_seen is not None:
            ...
            self._bridge_heartbeats.pop(client_id, None)
        if status == BridgeClientStatus.DISABLED.value:
            return BridgeClientLiveness.OFFLINE
        if principal_id is not None and stored_principal != principal_id:
            return BridgeClientLiveness.OFFLINE
    ...
    def _durable_bridge_liveness_row(self, client_id: str) -> sqlite3.Row | None:
        if self._connection is None or not self._database_path.exists():
            return None
        ...
        try:
            ...
            return connection.execute(
                """
                SELECT principal_id, capability_fingerprint, last_seen_at, status
                  FROM bridge_clients
                 WHERE client_id = ?
                """,
                (client_id,),
    ...
    def _require_live_bridge_client(
        self,
        ...
        *,
        ...
        liveness = self.bridge_client_liveness(
            client.client_id,
            now=now,
            principal_id=client.principal_id,
            fingerprint=client.capability_fingerprint,
        )
        if liveness is not BridgeClientLiveness.LIVE:
            raise StateError(f"offline or expired Bridge client cannot {action}")
        ...
        *,
        ...
        async with self.transaction() as connection:
            ...
            if client.workspace_id != publication.workspace_id:
                raise StateError(
                    "Bridge manifest workspace does not match the registered client"
                )
            ...
            hash_row = await self._fetch_one(
                "SELECT s.* FROM snapshots AS s "
                "JOIN workspaces AS w ON w.workspace_id = s.workspace_id "
                "WHERE s.manifest_hash = ? AND w.owner_id = ?",
                (manifest.manifest_hash, principal_id),
            )
            if existing_row is not None:
                ...
                if existing.status is not SnapshotStatus.READY:
                    raise StateError("Bridge manifest requires a ready snapshot")
                ...
                if existing_manifest.manifest_hash != manifest.manifest_hash:
                    raise StateError(
                        "stale Bridge manifest cannot replace a published snapshot"
                    )
                persisted = existing
                persisted_manifest = existing_manifest
            else:
                if hash_row is not None:
                    if str(hash_row["workspace_id"]) != publication.workspace_id:
                        raise DuplicateEntityError(
                            "manifest hash belongs to another workspace"
                        )
                    raise StateError(
                        "stale Bridge manifest cannot replace a published snapshot"
                    )
            ...
            owner_row = await self._fetch_one(
                """
                SELECT client_id FROM bridge_clients
                 WHERE current_snapshot_id = ? AND client_id <> ?
                """,
                (persisted.snapshot_id, client.client_id),
            )
            if owner_row is not None:
                raise StateError("Bridge snapshot already belongs to another client")
            if client.current_snapshot_id != persisted.snapshot_id:
                try:
                    await connection.execute(
                        """
                        UPDATE bridge_clients
                           SET current_snapshot_id = ?
                         WHERE client_id = ? AND principal_id = ? AND workspace_id = ?
                        """,
                        (
                    ...
                    raise StateError(
                        "Bridge snapshot already belongs to another client"
                    ) from error
            return BridgeManifestAcceptance(
                ...
                file_count=len(persisted_manifest.entries),
                total_size=persisted_manifest.total_size,
    ...
    async def bind_bridge_call_snapshot(
        ...
        *,
        ...
        """Point one owner-scoped tool call at the selected client's current snapshot."""
        if not snapshot_id or snapshot_id != snapshot_id.strip():
            raise ValueError("snapshot_id must identify one READY snapshot")
        async with self.transaction() as connection:
            row = await self._fetch_one(
                """
                SELECT tc.call_id, tc.workspace_id, tc.base_snapshot_id
                  FROM tool_calls AS tc
                  JOIN session_runs AS sr ON sr.run_id = tc.run_id
                  JOIN sessions AS s ON s.session_id = sr.session_id
                  JOIN workspaces AS w ON w.workspace_id = tc.workspace_id
                 WHERE tc.call_id = ? AND s.principal_id = ? AND w.owner_id = ?
                """,
                (call_id, principal_id, principal_id),
            )
            if row is None:
                raise MissingEntityError("tool call is outside the authenticated Bridge scope")
            snapshot_row = await self._fetch_one(
                """
                SELECT s.snapshot_id, s.workspace_id, s.status
                  FROM snapshots AS s
                  JOIN workspaces AS w ON w.workspace_id = s.workspace_id
                 WHERE s.snapshot_id = ? AND w.owner_id = ?
                """,
                (snapshot_id, principal_id),
            )
            if snapshot_row is None:
                raise MissingEntityError("Bridge snapshot is not available")
            if str(snapshot_row["workspace_id"]) != str(row["workspace_id"]):
                raise StateError("Bridge snapshot workspace does not match the call")
            if SnapshotStatus(str(snapshot_row["status"])) is not SnapshotStatus.READY:
                raise StateError("Bridge claim requires a ready client snapshot")
            if str(row["base_snapshot_id"]) == snapshot_id:
                return
            await connection.execute(
                "UPDATE tool_calls SET base_snapshot_id = ? WHERE call_id = ?",
                (snapshot_id, call_id),
            )
    ...
    async def claim_tool_call(
        ...
        *,
        ...
        async with self.transaction() as connection:
            ...
            self._require_live_bridge_client(client, now=now, action="claim")
            if client.workspace_id != str(row["workspace_id"]):
                raise StateError("Bridge client workspace does not match the call")
            if (
                client.current_snapshot_id is not None
                and client.current_snapshot_id != call.snapshot_id
            ):
                raise StateError(
                    "Bridge claim snapshot does not match the client's current snapshot"
                )
    ...
    async def renew_bridge_claim(
        ...
        *,
        ...
        async with self.transaction() as connection:
            ...
            renew_client = await self.get_bridge_client(
                current.client_id, principal_id=principal_id
            )
            self._require_live_bridge_client(renew_client, now=at, action="renew")
        ...
        *,
        ...
        async with self.transaction() as connection:
            ...
            if snapshot_id is not None and snapshot_id != claim.snapshot_id:
                raise StateError("Bridge result base snapshot does not match the assigned call")
            result_client = await self.get_bridge_client(
                claim.client_id, principal_id=principal_id
            )
            self._require_live_bridge_client(
                result_client, now=settlement_time, action="settle"
            )
            if (
                result_client.current_snapshot_id is not None
                and result_client.current_snapshot_id != claim.snapshot_id
            ):
                raise StateError(
                    "Bridge result snapshot does not match the client's current snapshot"
                )
            snapshot_row = await self._fetch_one(
    ...
    def _row_to_bridge_client(self, row: aiosqlite.Row) -> RegisteredBridgeClient:
        ...
        current_raw = row["current_snapshot_id"]
        client = RegisteredBridgeClient(
            client_id=str(row["client_id"]),
            ...
            current_snapshot_id=None if current_raw is None else str(current_raw),
        )
        return client
