...
async def _assigned_bridge_client(
    ...
    *,
    ...
    if require_live:
        liveness = _store(request).bridge_client_liveness(
            client.client_id,
            now=utc_now(),
...
def create_router() -> APIRouter:
    ...
    async def claim_tool_call(
        ...
        if (
            ...
            and body.snapshot_id is not None
            and body.snapshot_id != client.current_snapshot_id
        ):
            raise HTTPException(
                status_code=409,
                detail={
                    "code": "stale_bridge_snapshot",
                    "message": "Bridge claim requires the client's current snapshot",
                },
            )
        ...
        try:
            assignment = await _coordinator(request).assign_for_call(
                current_call,
                principal_id=auth.principal_id,
                workspace_id=session.workspace_id,
                session_id=session_id,
                idempotency_key=idempotency_key,
                caller_client_id=client.client_id,
                claimed_at=utc_now(),
            )
        except MissingEntityError as error:
            raise _not_found("tool call is not available for Bridge claim") from error
            ...
            if "server-selected" in str(error):
                raise _bridge_forbidden(
                    "bridge_client_mismatch",
                    "only the server-selected Bridge client can claim this call",
                ) from error
            raise HTTPException(
                status_code=409,
                detail={
        ...
        if assignment.claim is None:
            raise HTTPException(
                status_code=409,
                detail={
                    "code": "bridge_claim_unavailable",
                    "message": "no eligible Bridge client was selected for this call",
                },
            )
    ...
    async def renew_tool_claim(
        ...
        try:
            ...
            if (
                client.current_snapshot_id is not None
                and current.snapshot_id != client.current_snapshot_id
            ):
                raise HTTPException(
                    status_code=409,
                    detail={
                        "code": "stale_bridge_snapshot",
                        "message": "Bridge renew requires the client's current snapshot",
                    },
                )
    ...
    async def submit_tool_result(
        ...
        if (
            ...
            raise HTTPException(
                ...
                detail={
                    "code": "stale_bridge_snapshot",
                    "message": "Bridge result requires the client's current snapshot",
                },
            )
        if (
            client.current_snapshot_id is not None
