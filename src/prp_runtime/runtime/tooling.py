...
def _canonical_bridge_capabilities() -> ClientCapabilityDescriptor:
    tools = tuple(sorted(_BRIDGE_TOOL_EFFECTS))
    effects = tuple(
        sorted(
            {_BRIDGE_TOOL_EFFECTS[name] for name in tools},
            key=lambda item: item.value,
        )
    )
    return ClientCapabilityDescriptor(tools=tools, effects=effects)
...
class BridgeRemoteToolExecutor:
    ...
    async def provider_catalog(self) -> tuple[ProviderToolDescriptor, ...]:
        """Return the union of active client catalogs without selecting one."""

        clients = await self._active_clients()
        seen: dict[str, ProviderToolDescriptor] = {}
        for client in clients:
            for descriptor in catalog_from_bridge_capabilities(client.capabilities):
                seen.setdefault(descriptor.name, descriptor)
    ...
    async def execute(
        ...
        *,
        ...
        try:
            registry = _bridge_sentinel_registry(_canonical_bridge_capabilities())
        ...
        if (
            ...
            raise RemoteToolAssignmentPending(outcome.assignment)
    ...
    async def _active_clients(self) -> tuple[RegisteredBridgeClient, ...]:
    ...
    async def _ready_snapshot_id(self) -> SnapshotId:
        ...
        for client in sorted(clients, key=lambda item: item.client_id):
            if client.current_snapshot_id:
                return client.current_snapshot_id
        raise ToolRuntimeError("BRIDGE requires a published client snapshot")
