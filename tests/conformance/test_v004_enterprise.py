...
def test_fake_enterprise_two_client_server_brain_loop(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        with WorkspaceBackend(workspace_root) as backend:
            published_manifest = backend.snapshot_manifest()
        published = client.post(
            ...
            json={
                ...
                "entries": [
                    ...
                    for entry in published_manifest.entries
                ],
                "manifest_hash": published_manifest.manifest_hash,
            },
            headers=headers,
        )
        assert published.status_code == 201, published.text
        assert published.json()["snapshot_id"] == snapshot_id
        session = client.post(
            "/v1/sessions",
            headers=headers,
            json={
