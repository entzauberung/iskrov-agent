...
def test_supervisor_liveness_follows_store_not_empty_memory_cache() -> None:
    from datetime import UTC, datetime, timedelta

    from prp_runtime.domain.enums import BridgeClientLiveness

    current = datetime(2026, 9, 2, 15, 0, tzinfo=UTC)
    ...
    class DurableStore(FakeStore):
        ...
        def bridge_client_liveness(
            ...
            *,
            ...
            del client_id
            if self.status != "ACTIVE" or self.last_seen is None:
                return BridgeClientLiveness.OFFLINE
            if fingerprint is not None and fingerprint != self.fingerprint:
                return BridgeClientLiveness.OFFLINE
            if now - self.last_seen > timedelta(seconds=30):
                return BridgeClientLiveness.EXPIRED
            return BridgeClientLiveness.LIVE

    store = DurableStore()
    supervisor = RunSupervisor(
        store, lambda run_id: run_id, heartbeat_ttl=30.0, clock=lambda: current
    )
    assert supervisor._heartbeats == {}
    assert (
        supervisor.bridge_client_liveness("cli_durable01", fingerprint="a" * 64)
        is BridgeClientLiveness.LIVE
    )
    supervisor.record_bridge_heartbeat(
        "cli_durable01", fingerprint="a" * 64, at=current - timedelta(seconds=60)
    )
    assert (
        supervisor.bridge_client_liveness("cli_durable01", fingerprint="a" * 64)
        is BridgeClientLiveness.LIVE
    )
    store.last_seen = current - timedelta(seconds=31)
    assert supervisor.bridge_client_liveness("cli_durable01") is BridgeClientLiveness.EXPIRED
    store.status = "DISABLED"
    assert supervisor.bridge_client_liveness("cli_durable01") is BridgeClientLiveness.OFFLINE
