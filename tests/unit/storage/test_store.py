...
async def test_bridge_manifest_publication_is_client_scoped_and_rejects_stale(
    ...
    bound = await store.get_bridge_client(client.client_id, principal_id="prn_bridge")
    assert bound.current_snapshot_id == publication.snapshot_id
    replay = await store.publish_bridge_manifest(
        publication, principal_id="prn_bridge", published_at=T0
    )
    assert replay.snapshot_id == accepted.snapshot_id
    replayed = await store.get_bridge_client(client.client_id, principal_id="prn_bridge")
    assert replayed.current_snapshot_id == publication.snapshot_id
...
@pytest.mark.asyncio
async def test_bridge_clients_keep_distinct_current_snapshots_and_reject_cross_binding(
    ...
    client_b = await register_scoped_client(store, session)
    first = BridgeManifestPublication(
        snapshot_id=new_snapshot_id(),
        client_id=client_a.client_id,
        workspace_id=workspace.workspace_id,
        entries=make_manifest(suffix="-client-a").entries,
    )
    second = BridgeManifestPublication(
        snapshot_id=new_snapshot_id(),
        client_id=client_b.client_id,
        workspace_id=workspace.workspace_id,
        entries=make_manifest(suffix="-client-b").entries,
    )
    accepted_a = await store.publish_bridge_manifest(
        first, principal_id="prn_bridge", published_at=T0
    )
    accepted_b = await store.publish_bridge_manifest(
        second, principal_id="prn_bridge", published_at=T0
    )
    assert accepted_a.snapshot_id != accepted_b.snapshot_id
    stolen = first.model_copy(update={"client_id": client_b.client_id})
    with pytest.raises(StateError, match="another client"):
        await store.publish_bridge_manifest(stolen, principal_id="prn_bridge")
    await store.close()
    async with SqliteStore(database_path) as reopened:
        restored_a = await reopened.get_bridge_client(
            client_a.client_id, principal_id="prn_bridge"
        )
        restored_b = await reopened.get_bridge_client(
            client_b.client_id, principal_id="prn_bridge"
        )
        assert restored_a.current_snapshot_id == accepted_a.snapshot_id
        assert restored_b.current_snapshot_id == accepted_b.snapshot_id
        assert restored_a.current_snapshot_id != restored_b.current_snapshot_id


@pytest.mark.asyncio
async def test_bridge_call_bind_and_claim_reject_other_client_current(
    store: SqliteStore,
) -> None:
    ...
    first = BridgeManifestPublication(
        ...
        entries=make_manifest(suffix="-bind-a").entries,
    )
    second = BridgeManifestPublication(
        snapshot_id=new_snapshot_id(),
        client_id=client_b.client_id,
        workspace_id=session.workspace_id,
        entries=make_manifest(suffix="-bind-b").entries,
    )
    accepted_a = await store.publish_bridge_manifest(
        first, principal_id="prn_bridge", published_at=T0
    )
    ...
    await store.bind_bridge_call_snapshot(
        call.call_id,
        snapshot_id=accepted_a.snapshot_id,
        principal_id="prn_bridge",
    )
    rebound = await store.get_tool_call(call.call_id)
    assert rebound.snapshot_id == accepted_a.snapshot_id
    assert rebound.snapshot_id != accepted_b.snapshot_id
    with pytest.raises(StateError, match="current snapshot"):
        await store.claim_tool_call(
            ...
            principal_id="prn_bridge",
            client_id=client_b.client_id,
            idempotency_key="claim-other-current",
            claimed_at=T0,
        )
    claimed = await store.claim_tool_call(
        session.session_id,
        run.run_id,
        call.call_id,
        principal_id="prn_bridge",
        client_id=client_a.client_id,
        idempotency_key="claim-own-current",
        claimed_at=T0,
    )
    assert claimed.client_id == client_a.client_id
    assert claimed.snapshot_id == accepted_a.snapshot_id
    with pytest.raises(StateError, match="snapshot"):
        await store.submit_bridge_tool_result(
            ...
            principal_id="prn_bridge",
            client_id=client_a.client_id,
            snapshot_id=accepted_b.snapshot_id,
...
async def register_scoped_client(
    ...
    *,
    ...
    registered = await store.register_bridge_client(client)
...
@pytest.mark.asyncio
async def test_bridge_liveness_reads_durable_last_seen_after_restart(
    database_path: Path,
) -> None:
    from prp_runtime.domain.enums import BridgeClientLiveness

    async with SqliteStore(database_path) as store:
        _, session, _ = await seed_bridge_tool_call(store)
        unpublished = ClientCapabilityDescriptor(
            tools=("read_file",), effects=(ToolEffect.READ,)
        )
        client = await store.register_bridge_client(
            RegisteredBridgeClient(
                client_id=new_client_id(),
                principal_id=session.principal_id,
                workspace_id=session.workspace_id,
                capabilities=unpublished,
                capability_fingerprint=fingerprint_client_capabilities(unpublished),
                created_at=T0,
            )
        )
        live = await register_scoped_client(
            store, session, tools=("list_files",), effects=(ToolEffect.READ,)
        )
        store._bridge_heartbeats[client.client_id] = {
            "principal_id": session.principal_id,
            "fingerprint": client.capability_fingerprint,
            "last_seen": T0,
            "status": "ACTIVE",
        ...
        assert (
            store.bridge_client_liveness(
                client.client_id,
                now=T0,
                principal_id=session.principal_id,
                fingerprint=client.capability_fingerprint,
            )
            is BridgeClientLiveness.OFFLINE
        )
        observed = T0 + timedelta(seconds=10)
        for item in (client, live):
            await store.record_bridge_heartbeat(
                item.client_id,
                principal_id=session.principal_id,
                fingerprint=item.capability_fingerprint,
                at=observed,
            )
        store._bridge_heartbeats.clear()
        assert (
            store.bridge_client_liveness(
                client.client_id,
                now=observed,
        ...
        disabled_id = client.client_id
        disabled_fp = client.capability_fingerprint
        live_id = live.client_id
        live_fp = live.capability_fingerprint
        principal_id = session.principal_id
    async with SqliteStore(database_path) as reopened:
        assert reopened._bridge_heartbeats == {}
        reopened._bridge_heartbeats[disabled_id] = {
        ...
        assert (
            reopened.bridge_client_liveness(
                disabled_id,
...
@pytest.mark.asyncio
async def test_expired_bridge_client_cannot_claim_renew_or_settle(
    store: SqliteStore,
) -> None:
    run, session, call = await seed_bridge_tool_call(store)
    client = await register_scoped_client(store, session)
    ...
    with pytest.raises(StateError, match="offline or expired"):
        await store.claim_tool_call(
            session.session_id,
            run.run_id,
            call.call_id,
            ...
            claimed_at=T0 + timedelta(seconds=31),
        )
    claim = await store.claim_tool_call(
        session.session_id,
        run.run_id,
        ...
        principal_id=session.principal_id,
        client_id=client.client_id,
        idempotency_key="live-then-expire",
        claimed_at=T0,
        expires_at=T0 + timedelta(seconds=60),
    )
    with pytest.raises(StateError, match="offline or expired"):
        await store.renew_bridge_claim(
            claim.claim_id,
            principal_id=session.principal_id,
    ...
    with pytest.raises(StateError, match="offline or expired"):
        await store.submit_bridge_tool_result(
            ...
            client_id=client.client_id,
            snapshot_id=call.snapshot_id,
            settled_at=T0 + timedelta(seconds=31),
        )
