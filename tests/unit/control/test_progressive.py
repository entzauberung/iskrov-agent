...
@pytest.mark.asyncio
async def test_controller_assigns_pending_bridge_call_to_coordinator_selected_client(
    store: SqliteStore,
) -> None:
    ...
    from prp_runtime.domain.values import ModelRef, new_attempt_id, new_tool_call_id, utc_now
    from prp_runtime.runtime.worker import WorkerResult
    ...
    workspace = make_workspace(owner_id="prn_operator")
    ...
    await store.attach_run_to_session(
        session.session_id, run.run_id, principal_id=workspace.owner_id
    )
    ...
    await store.create_tool_call(
        call, workspace_id=workspace.workspace_id, idempotency_key=call.call_id
    )
    started = await store.start_tool_call(call.call_id, started_at=T0)
    assert started.status is ToolCallStatus.RUNNING

    reader_capabilities = ClientCapabilityDescriptor(
        tools=("read_file",), effects=(ToolEffect.READ,)
    )
    writer_capabilities = ClientCapabilityDescriptor(
        tools=("apply_patch",), effects=(ToolEffect.WRITE,)
    )
    reader = RegisteredBridgeClient(
        client_id=new_client_id(),
        principal_id=workspace.owner_id,
        workspace_id=workspace.workspace_id,
        capabilities=reader_capabilities,
        capability_fingerprint=fingerprint_client_capabilities(reader_capabilities),
        created_at=T0,
        last_seen_at=utc_now(),
    )
    writer = RegisteredBridgeClient(
        client_id=new_client_id(),
        principal_id=workspace.owner_id,
        workspace_id=workspace.workspace_id,
        capabilities=writer_capabilities,
        capability_fingerprint=fingerprint_client_capabilities(writer_capabilities),
        created_at=T0,
        last_seen_at=utc_now(),
    )
    await store.register_bridge_client(reader)
    await store.register_bridge_client(writer)
    observed = utc_now()
    ...
    await controller._assign_bridge_pending_calls(
        run, unit, result, execution_scope=scope
    )
    claim = await store.get_bridge_claim_for_session(
        session.session_id, run.run_id, call.call_id, principal_id=workspace.owner_id
    )
    assert claim.client_id == reader.client_id
    assert claim.client_id != writer.client_id
    assert claim.call_id == call.call_id
    assert claim.workspace_id == workspace.workspace_id
    assert claim.snapshot_id == snapshot.snapshot_id
    replay = await controller._assign_bridge_pending_calls(
        run, unit, result, execution_scope=scope
    )
    del replay
    replayed = await store.get_bridge_claim_for_session(
        session.session_id, run.run_id, call.call_id, principal_id=workspace.owner_id
    )
    assert replayed.claim_id == claim.claim_id
