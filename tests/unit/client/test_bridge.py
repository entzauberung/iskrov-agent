...
async def test_write_crash_persists_pending_and_cannot_rerun(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert pending["claimed_calls"]["call-write"]["execution"]["status"] == "PENDING"
    ...
    saved = json.loads(state_path.read_text(encoding="utf-8"))
    execution = saved["claimed_calls"]["call-write"]["execution"]
    assert execution["status"] == "UNKNOWN"
    assert execution["version"] == 1
    assert execution["call_id"] == "call-write"
    assert "token" not in state_path.read_text(encoding="utf-8")
...
@pytest.mark.asyncio
async def test_confirmed_write_replays_fingerprint_and_changeset_after_restart(
    tmp_path: Path,
) -> None:
    result_requests = 0
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/result"):
            result_requests += 1
            return httpx.Response(
                200, json={"call_id": "call-write", "status": "SUCCEEDED"}
            )
        raise AssertionError(f"unexpected request {request.url.path}")
    ...
    state_path = tmp_path / "confirmed-state.json"
    client_id = new_client_id()
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    bridge = Bridge("https://bridge.test", "token", state_path=state_path, client=client)
    bridge.state.session_id = "sess-1"
    ...
    try:
        first = await bridge.execute_claim(claim, executor)
        resumed = Bridge(
            "https://bridge.test", "token", state_path=state_path, client=client
        )
        replay = await resumed.execute_claim(claim, executor)
        foreign = dict(claim)
        foreign["snapshot_id"] = "snap_other"
        with pytest.raises(BridgeStateError, match="original claim"):
            await resumed.execute_claim(foreign, executor)
    finally:
        await _close(client)
    ...
    saved = json.loads(state_path.read_text(encoding="utf-8"))
    execution = saved["claimed_calls"]["call-write"]["execution"]
    expected_payload = dict(executor.payload)
    expected_payload["dev_only"] = True
    assert execution["status"] == "CONFIRMED"
    assert execution["version"] == 1
    assert execution["change_set_id"] == "cs_confirmed01"
    assert execution["claim_id"] == "claim-1"
    assert execution["snapshot_id"] == "snap_base"
    assert execution["client_id"] == client_id
    assert execution["payload"] == expected_payload
    assert execution["fingerprint"] == hashlib.sha256(
        json.dumps(expected_payload, ensure_ascii=True, separators=(",", ":"), sort_keys=True).encode("utf-8")
    ).hexdigest()


@pytest.mark.asyncio
async def test_pending_write_is_not_promoted_by_generic_submitted_result() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        raise AssertionError(f"unexpected request {request.url.path}")
    ...
    bridge.state.run_id = "run-1"
    bridge.state.claimed_calls["call-write"] = {"execution": {"status": "PENDING"}}
    bridge.state.submitted_results["call-write"] = {
        "fingerprint": "a" * 64,
        "response": {"status": "SUCCEEDED", "call_id": "call-write"},
    }
    executor = _CountingExecutor()
    try:
        with pytest.raises(BridgeStateError, match="UNKNOWN/BLOCKED"):
            await bridge.execute_claim(
                {"call_id": "call-write", "effect": "WRITE"},
                executor,
            )
        ...
        await _close(client)
    assert executor.count == 0
    assert bridge.state.claimed_calls["call-write"]["execution"]["status"] == "UNKNOWN"


def test_execution_record_version_is_validated() -> None:
    with pytest.raises(BridgeStateError, match="unsupported local execution record version"):
        BridgeState.from_json(
            {
                "version": 1,
                "claimed_calls": {
                    "call-1": {
                        "execution": {
                            "version": 99,
                            "status": "CONFIRMED",
                            "payload": {"dev_only": True, "status": "SUCCEEDED"},
                            "fingerprint": "0" * 64,
                        }
                    }
                },
