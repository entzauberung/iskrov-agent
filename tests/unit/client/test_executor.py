...
@pytest.mark.asyncio
async def test_patch_result_keeps_changeset_identity_on_in_process_replay(
    tmp_path: Path,
) -> None:
    source = tmp_path / "src"
    source.mkdir()
    ...
    with WorkspaceBackend(tmp_path) as backend:
        executor = BridgeExecutor(build_bridge_registry(backend, workspace_root=tmp_path), tmp_path)
        first = await executor.execute(claim, now=NOW)
    assert first["status"] == "SUCCEEDED"
    assert first["result"]["change_set_id"]
    assert first["result"]["base_snapshot_id"] == snapshot_id
    assert first["changed_paths"] == ["src/main.py"]
    assert (source / "main.py").read_text(encoding="utf-8") == "new\n"
