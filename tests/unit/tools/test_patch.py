...
@pytest.mark.asyncio
async def test_local_patch_store_replays_confirmed_identity_and_empty_store_stays_stale(
    tmp_path: Path,
) -> None:
    target = tmp_path / "main.py"
    target.write_text("old\n", encoding="utf-8")
    ...
    with WorkspaceBackend(tmp_path) as backend:
        ...
        first = await runner.apply(call, modification(base.snapshot_id, "main.py"))
        replay = await runner.apply(call, modification(base.snapshot_id, "main.py"))
        confirmed = await store.list_change_sets(tool_call_id=call.call_id)

        assert replay == first
        assert replay.change_set_id == confirmed[0].change_set_id
        assert target.read_text(encoding="utf-8") == "new\n"

        empty = LocalPatchStore()
        restarted = PatchRunner(
            backend,
            empty,
            owner_id=OWNER_ID,
            base_snapshot=base,
        ...
        with pytest.raises(PatchStaleError, match="no longer matches"):
            await restarted.apply(call, modification(base.snapshot_id, "main.py"))
        assert await empty.list_change_sets(tool_call_id=call.call_id) == ()
