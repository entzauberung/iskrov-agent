...
def test_bridge_claim_rejects_client_that_was_not_server_selected(
    tmp_path: Path,
) -> None:
    workspace_root = tmp_path / "bridge-workspace"
    workspace_root.mkdir()
    ...
    session_id, run_id, call_id, _snapshot_id = seed_running_bridge_call(
        database_path, workspace_root=workspace_root, call_id="tc_bridge_selected"
    )
    first = _handshake_body("read_file")
    second = _handshake_body("read_file")
    selected_id, other_id = sorted((str(first["client_id"]), str(second["client_id"])))
    with TestClient(app) as client:
        ...
        path = f"/v1/sessions/{session_id}/runs/{run_id}/tool-calls/{call_id}/claim"
        rejected = client.post(
            path,
            json={"call_id": call_id, "client_id": other_id},
            headers={**auth_headers(), "Idempotency-Key": "claim-other"},
        )
        assert rejected.status_code == 403
        assert rejected.json()["detail"]["code"] == "bridge_client_mismatch"
        accepted = client.post(
            path,
            json={"call_id": call_id, "client_id": selected_id},
            headers={**auth_headers(), "Idempotency-Key": "claim-selected"},
        )
        assert accepted.status_code == 201, accepted.json()
        assert accepted.json()["client_id"] == selected_id
        assert accepted.json()["call_id"] == call_id
        replay_rejected = client.post(
            path,
            json={"call_id": call_id, "client_id": other_id},
            headers={**auth_headers(), "Idempotency-Key": "claim-other-replay"},
        )
        assert replay_rejected.status_code == 403
        assert replay_rejected.json()["detail"]["code"] == "bridge_client_mismatch"
...
def test_expired_client_cannot_settle_after_store_ttl(tmp_path: Path) -> None:
    workspace_root = tmp_path / "bridge-workspace"
    workspace_root.mkdir()
    (workspace_root / "README.md").write_text("fixture read\n", encoding="utf-8")
    app = build_app(tmp_path, workspace_root=workspace_root)
    database_path = tmp_path / "agent.db"
    session_id, run_id, call_id, _snapshot_id = seed_running_bridge_call(
        database_path, workspace_root=workspace_root, call_id="tc_bridge_late"
    )
    payload = _handshake_body("read_file")
    with TestClient(app) as client:
        assert (
            client.post(
                "/v1/bridge/clients",
        ...
        claimed = client.post(
            ...
            headers={**auth_headers(), "Idempotency-Key": "claim-late"},
        )
        ...
        late = client.post(
            ...
            headers={**auth_headers(), "Idempotency-Key": "result-late"},
        )
        assert late.status_code == 403
        assert late.json()["detail"]["code"] == "bridge_client_offline"
...
def test_bridge_clients_publish_distinct_current_snapshots_that_survive_restart(
    tmp_path: Path,
) -> None:
    app = build_app(tmp_path)
    first = _handshake_body("read_file")
    ...
    with TestClient(app) as client:
        for payload, manifest in manifests:
            assert (
                client.post(
                    "/v1/bridge/clients",
                    json=_registration_body(payload),
            ...
            published[str(payload["client_id"])] = response.json()["snapshot_id"]
        listed = client.get("/v1/bridge/clients", headers=auth_headers())
        assert listed.status_code == 200
        bodies = {item["client_id"]: item for item in listed.json()}
        assert bodies[first["client_id"]]["current_snapshot_id"] == published[first["client_id"]]
        assert bodies[second["client_id"]]["current_snapshot_id"] == published[second["client_id"]]
        assert (
            bodies[first["client_id"]]["current_snapshot_id"]
            != bodies[second["client_id"]]["current_snapshot_id"]
        )
        stolen = client.post(
            "/v1/bridge/manifests",
            json={
                "snapshot_id": published[first["client_id"]],
    ...
    async def restore() -> None:
        async with SqliteStore(tmp_path / "agent.db") as store:
            restored_first = await store.get_bridge_client(
                str(first["client_id"]), principal_id="prn_operator"
            )
            restored_second = await store.get_bridge_client(
                str(second["client_id"]), principal_id="prn_operator"
            )
            assert restored_first.current_snapshot_id == published[first["client_id"]]
            assert restored_second.current_snapshot_id == published[second["client_id"]]

    asyncio.run(restore())


def test_assigned_bridge_client_cannot_use_other_client_current_snapshot(
    tmp_path: Path,
) -> None:
    workspace_root = tmp_path / "bridge-workspace"
    workspace_root.mkdir()
    ...
    app = build_app(tmp_path, workspace_root=workspace_root)
    database_path = tmp_path / "agent.db"
    session_id, run_id, call_id, _seed_snapshot_id = seed_running_bridge_call(
        database_path, workspace_root=workspace_root, call_id="tc_bridge_split_snap"
    )
    first = _handshake_body("read_file")
    second = _handshake_body("read_file")
    selected_id, other_id = sorted((str(first["client_id"]), str(second["client_id"])))
    ...
    with TestClient(app) as client:
        ...
        rejected = client.post(
            path,
            json={"call_id": call_id, "client_id": other_id},
            headers={**auth_headers(), "Idempotency-Key": "claim-other-current"},
        )
        assert rejected.status_code == 403
        assert rejected.json()["detail"]["code"] == "bridge_client_mismatch"
        stale_claim = client.post(
            ...
            json={
                ...
                "snapshot_id": published[other_id],
            },
            headers={**auth_headers(), "Idempotency-Key": "claim-selected-stale"},
        )
        assert stale_claim.status_code == 409
        assert stale_claim.json()["detail"]["code"] == "stale_bridge_snapshot"
        accepted = client.post(
            path,
            json={
                "call_id": call_id,
                "client_id": selected_id,
                "snapshot_id": published[selected_id],
            },
            headers={**auth_headers(), "Idempotency-Key": "claim-selected-current"},
        )
        assert accepted.status_code == 201, accepted.json()
        assert accepted.json()["client_id"] == selected_id
        assert accepted.json()["snapshot_id"] == published[selected_id]
        stale_renew = client.post(
            f"{path}/renew",
            json={"client_id": other_id},
            headers=auth_headers(),
        )
        assert stale_renew.status_code == 403
        assert stale_renew.json()["detail"]["code"] == "bridge_client_mismatch"
        stale_result = client.post(
            ...
            headers={**auth_headers(), "Idempotency-Key": "result-other-current"},
        )
        assert stale_result.status_code == 409
        assert stale_result.json()["detail"]["code"] == "stale_bridge_snapshot"
