...
Iskrov Agent 是一个云端控制、本机执行工具的 Agent 产品。服务器负责模型、规划、审批、证据、预算、恢复和调度；本机只运行轻量、封闭、授权的工具客户端。渐进式推理协议是它实现的一个策略，不是本仓库的独立身份。

当前版本：`0.0.1` · Python `3.12+` · 单实例 SQLite · 无 Docker · AGPL-3.0-only
...
本项目采用 AGPL-3.0-only，详见 [LICENSE](LICENSE)、[NOTICE](NOTICE) 和 [TRADEMARKS.md](TRADEMARKS.md)。当前版本是单实例 SQLite 参考实现，不提供多租户计费、SSO、分布式队列、Kubernetes、完整 Codex/Claude Code/MCP/A2A 兼容或生产 SLA。
