...
- Unified package, runtime, tests, README, and release metadata on `0.1.1`.
- Kept `iskrov` and `iskrov-bridge` as official CLI entry points.
...
- Renamed the product to Iskrov Agent.
- Switched the product license to AGPL-3.0-only.
- Separated the Progressive Reasoning Protocol into the independent Apache-2.0 `prp-protocol` repository.
...
- v0.1.0 package identity、最小云端代码任务（list/search/read/patch/targeted test/diff）和 API/Bridge 定向门禁的源码与定向证据已记录。
